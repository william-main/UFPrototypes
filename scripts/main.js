...
var map;
var articlesToSkip = 4;
var mapsKey = "AIzaSyCOj9geycjeT87bvazfZvwVRxuByaIORdE";
...
(function() {
	// makes external SVG links work on IE
  	...
  	$(document).ready(function () {
  	    ...
  	    //store the google maps key to be used on API calls
	      mapsKey = $("#mapsAPIKey").val();
		...
		transitionsInit($(document).scrollTop(), 0);
		var checkNumber = setInterval(function () {
		    if ($('.rTapNumber228290').attr('style')) {
		        var responseTapNumber = $(".rTapNumber228290").first().text().replace(/ /g, "");
		            clearInterval(checkNumber);
                    $('*[data-js-telno]').each(function () {
		                $(this).removeAttr("href");
                        $(this).attr("href", "tel:" + responseTapNumber);
                    });
		        } else {
		        }
		    },
...
function contactSuccess() {
    if ($('.contourMessageOnSubmit').length > 0) {
        ...
        var elTop = parseInt($('.contourMessageOnSubmit').offset().top);
        setTimeout(function () {
            $('html, body').animate({
                scrollTop: elTop
            }, 500);
...
// hack to allow for vimeo videos
function playVimeoVideo(vimeoId) {
    var btn = $('<a id="vimeoPopup" href="#vimeo-popup" data-js="modal-trigger" class="mfp-hide"></a>');
    var modal = $('<div id="vimeo-popup" class="white-popup mfp-hide"><div id="vimeoVid" class="video-container u-margin-bottom-sm u-margin-bottom-md-mob"></div></div>');
    $('body').after(btn);
    $('body').after(modal);
    var ifrm = document.createElement('iframe');
    ifrm.setAttribute('src', '//player.vimeo.com/video/' + vimeoId);
    $('#vimeoVid').append(ifrm);
    modalInit();
    $('#vimeoPopup').trigger('click');
}
...
function createMap(container, lat, lng) {
    ...
    var placeId = $('*[data-map-controller]').children('option:selected').attr('data-placeid');
    
    if (placeId !== '') {
        //build the embedded map
        var mapSrc = "https://www.google.com/maps/embed/v1/place?key=" + mapsKey + "&zoom=17&q=place_id:" + placeId;
        //iframe
        var $iframe = $('<div style="height: 100%; width: 100%; position: absolute; top: 0px; left: 0px; background-color: rgb(229, 227, 223);">' +
            '<iframe frameborder="0" style="border: 0" width="100%" height="100%" allowfullscreen></iframe>' + '</div>');
        $("#map").html($iframe);
        $("#map iframe").attr("src", mapSrc);
    } else {
        //use the old style mapping with no placeid
...
function formInit() {
    ...
    $('*[data-js-filter-dropdown]').change(function () {
          ...
          var selectedText = $(this).find(":selected").val().replace(/[^0-9a-zA-Z]+\s+/g, '').replace('-', '');
    ...
    $('*[data-js-filter-dropdown]').blur(function () {
        ...
        $(".fadeOutProduct").hide();
        if ($(".fadeInProduct")[0]) {
            $(".fadeInProduct").fadeTo(400, 1);
        } else {
            $("*[data-category]").fadeTo(1, 1);
        }
    });

    $('*[data-js-filter-dropdown]').click(function () {
        var selectedText = $(this).find(":selected").val().replace(/[^0-9a-zA-Z]+\s+/g, '').replace('-', '');
        if (currentDropDownSetting == selectedText && blurred != true) {
            $('*[data-js-filter-dropdown]').blur();
        } else {
            blurred = false;
        }
    ...
    $('*[data-js-ratesmonths]').change(function () {
        if (!$('body').hasClass('calculating')) {
            ...
            if ((parseInt($(this).val()) > 0) && (parseInt($(this).val()) < 13)) {
...
function flipperInit() {
    ...
    if (flipStatus == "False") {
        if ($('html').hasClass('IE')) {
            if ($('*[data-js-flipper-toggle]')[0] != null) {
            ...
            //af: DD-119 fix for flipper causing issues in IE
            //return false;
        } else {
...
function numberSpinnerInit() {
    if (parseInt($('*[data-js-ratesmonths]').val() > 0) && (parseInt($('*[data-js-ratesmonths]').val()) < 13)) {
        $('*[data-js-loanmonths]').html($('*[data-js-ratesmonths]').val());
    }
        ...
        $(this).click(function () {
            if (!$('body').hasClass('calculating')) {
                ...
                if (input.val() < 1 || isNaN(inputVal)) {
                    ...
                    input.val(parseFloat(input.val()) + 1);
                    $('*[data-js-loanmonths]').html($('*[data-js-ratesmonths]').val());
                    $('*[data-js-minus-control]').removeClass('number-input__control--disabled');
...
function heroBgInit() {
    ...
    $('*[data-background]').each(function () {
        ...
        if ($(this).parents('.site-section--callOutSplit__image').length == 0) {
            bgStyle = 'linear-gradient(rgba(0,0,0,.45),rgba(0,0,0,.45)),'
        } else {
            bgStyle = ''
        }
        ...
        if (!$(this).hasClass('site-section__bg-split')) {
            ...
            if (!$('html').hasClass('IE')) {
                ...
                if (windowWidth < 1200 && $(this).attr('data-background-mob') !== "") {

                    bg = $(this).attr('data-background-mob');
                    $(this).css({
                        'background-image': bgStyle + 'url(' + bg + ')',
...
function lazyLoading() {
    ...
    function leftColText(blurb, buttonText, imageLarge, imageSmall, showcaseHeading, pageURL) {
        ...
        '<div class="container-fluid site-section--callOutSplit__image">' +
            '<div class="row">' +
                '<div class="col-md-6 col-md-offset-6" data-background="' + imageLarge + '" data-background-mob="' + imageSmall + '">' +
                '</div>' +
            '</div>' +
        '</div>' +
    ...
    function rightColText(blurb, buttonText, imageLarge, imageSmall, showcaseHeading, pageURL) {
       ...
       '<div class="container-fluid site-section--callOutSplit__image">' +
            '<div class="row">' +
                '<div class="col-md-6 col-md-offset6" data-background="' + imageLarge + '" data-background-mob="' + imageSmall + '">' +
                '</div>' +
            '</div>' +
        '</div>' +
...
function chatTriggerInit() {
    ...
    if (chatEnabled) {
        ...
        //    $zopim.livechat.departments.setVisitorDepartment('Testing');
        //});

        //check to see if a trigger has been thrown from the zopim api
        //also handle unread counts on the button if the chat window is minimised
        $zopim(function () {
            var btn = $("#chatBtn");
            var btnDiv = $(".web-chat-button");
            $zopim.livechat.setOnUnreadMsgs(trigger);
            function trigger(unread) {
                if (unread > 0 && isChatting === true) {
                    btn.html("Chat - Unread (" + unread + ")");
                    btn.removeClass("btn--brand-tertiary");
                    btn.addClass("btn--brand-primary");
                    btnDiv.addClass("btn--btn-shake");
                    setTimeout(function() {
                            btnDiv.removeClass("btn--btn-shake");
                        }, 2000);
                }
                if (unread > 0 && isChatting === false) {
                    $('body').toggleClass('chatOpen');
                    $zopim.livechat.window.show();
                    isChatting = true;
                }
                if (unread === 0 && isChatting === true) {
                    btn.html("Chat to us");
                    btn.addClass("btn--brand-tertiary");
                    btn.removeClass("btn--brand-primary");
                }
            }
        });
    }
